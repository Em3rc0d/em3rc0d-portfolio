...
export function LocalizedSystems({ locale }: { locale: Locale }) {
  ...
  return <main id="main-content" lang={locale} tabIndex={-1}>
    ...
    <section className="container" aria-label={t.systems.selectedLabel}>{featured.map((system,i)=><article className={`system-encounter encounter-${i}`} key={system.slug} data-accent={system.accent}><div className="encounter-copy"><p className="eyebrow accent">{system.category}</p><h2>{system.name}</h2><p className="encounter-summary">{system.summary}</p><p className="encounter-built">{system.built}</p><p className="encounter-state">{system.state.label}</p><Link className="text-link" href={localePath(locale,`/systems/${system.slug}`)}>{t.common.explore} {system.name}<span aria-hidden="true">↗</span></Link></div><SystemArtifact system={system}/></article>)}</section>
    <section className="section container"><div className="section-head"><div><p className="eyebrow accent">{t.systems.further}</p><h2><Lines value={t.systems.furtherTitle}/></h2></div><p>{t.systems.furtherText}</p></div><div className="supporting-systems">{systems.filter((system)=>system.placement!=="FLAGSHIP").map((system)=><Link href={localePath(locale,`/systems/${system.slug}`)} key={system.slug}><span className="eyebrow">{system.placement === "R&D" ? t.systems.inDevelopment : system.placement.toLowerCase()}</span><div><h3>{system.name}</h3><p>{system.summary}</p><span className="support-state">{system.state.label}</span></div><span className="row-arrow" aria-hidden="true">↗</span></Link>)}</div></section>
...
export function LocalizedSystem({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return <main id="main-content" lang={locale} className="system-page" data-accent={system.accent} tabIndex={-1}>
    ...
    <section className="paper section" id="evidence"><div className="container"><div className="section-head"><div><p className="eyebrow">{t.system.inspect}</p><h2><Lines value={t.system.trust}/></h2></div><p>{t.system.proofIntro}</p></div>{evidence.length>0&&<div className="proof-list">{evidence.map((record)=><Link key={record.id} href={localePath(locale,`/evidence/${record.slug}`)}><span className="eyebrow">{record.type.toLowerCase()}</span><h3>{record.title}</h3><span aria-hidden="true">↗</span></Link>)}</div>}{system.sourceBoundary&&<p className="source-boundary">{system.sourceBoundary}</p>}{system.source&&<a className="text-link source-link" href={system.source.href} target="_blank" rel="noreferrer">{system.source.label} <span aria-hidden="true">↗</span></a>}</div></section>
...
export function localizedSystemStaticSlugs() {
  return localizedSystemCases("en").filter((system)=>system.publicability!=="PRIVATE").map((system)=>system.slug);
}

export function localizedSystemCount() {
  return localizedSystemCases("en").filter((system)=>system.publicability!=="PRIVATE").length;
}

export function publicSystemEvidenceCount() {
  return publicEvidenceRecords.length;
}
