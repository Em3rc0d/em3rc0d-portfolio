...
export function LocalizedSystem({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return <main id="main-content" lang={locale} className="system-page" data-accent={system.accent} tabIndex={-1}>
    ...
    <nav className="case-nav container" aria-label={`${system.name} ${polish.system.sections}`}><a href="#problem">{t.system.problem}</a><a href="#what-it-does">{t.system.built}</a><a href="#current-state">{t.system.state}</a><a href="#engineering">{t.system.engineering}</a><a href="#evidence">{t.common.proof}</a></nav>
    ...
    <section className="state-section section container" id="current-state"><p className="eyebrow accent">{t.system.state}</p><div className="split"><h2>{system.state.label}</h2><div className="case-body"><p>{system.state.detail}</p><p className="state-boundary">{system.state.boundary}</p><p className="state-boundary">{polish.system.impactBoundary}</p></div></div></section>
