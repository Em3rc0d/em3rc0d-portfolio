...
export function generateStaticParams() {
  ...
  return prefixedLocales.flatMap((locale) =>
    routeSegments.map((segments) => ({ locale, segments })),
  );
}
...
export async function generateMetadata({ params }: Props) {
  ...
  if (!section) {
    return pageMetadata(
      "Eduardo Merino — Full Stack Developer & Applied AI",
      t.home.proposition,
      "/",
      "/opengraph-image",
      locale,
  ...
  if (section === "systems" && !slug) {
    return pageMetadata(t.systems.title, t.systems.lead, "/systems", "/opengraph-image", locale);
  }
  if (section === "systems" && slug) {
    const system = findLocalizedSystem(locale, slug);
    return system
      ? pageMetadata(system.name, system.summary, `/systems/${slug}`, `/systems/${slug}/opengraph-image`, locale)
      : {};
  }

  if (section === "notes" && !slug) {
    return pageMetadata(t.notes.eyebrow, t.notes.lead, "/notes", "/opengraph-image", locale);
  }
  if (section === "notes" && slug) {
    const note = findLocalizedNote(locale, slug);
    return note ? pageMetadata(note.title, note.thesis, `/notes/${slug}`, "/opengraph-image", locale) : {};
  }

  if (section === "evidence" && !slug) {
    return pageMetadata(t.evidence.eyebrow, t.evidence.lead, "/evidence", "/opengraph-image", locale);
  }
  if (section === "evidence" && slug) {
    const record = publicEvidenceRecords.find((item) => item.slug === slug);
    return record ? pageMetadata(record.id, record.claim, `/evidence/${slug}`, "/opengraph-image", locale) : {};
  }

  if (section === "about" && !slug) {
    return pageMetadata("Sobre Eduardo Merino", t.about.personal, "/about", "/opengraph-image", locale);
  }
  if (section === "contact" && !slug) {
    return pageMetadata(locale === "es" ? "Contacto" : "Contato", t.contact.opening, "/contact", "/opengraph-image", locale);
  }
