...
export function generateStaticParams() {
  return getLocalizedPublicSystems("es").map((system) => ({ slug: system.slug }));
}

export async function generateMetadata({ params }: SystemPageProps): Promise<Metadata> {
  const { slug } = await params;
  const system = getLocalizedSystemBySlug(slug, "es");
  if (!system) return {};

  const title = `${system.name} — ${system.label}`;
  const routePath = `/systems/${system.slug}`;
  const spanishPath = `/es/systems/${system.slug}`;
  const canonicalUrl = absoluteSiteUrl(spanishPath);
  const socialImage = absoluteSiteUrl("/opengraph-image");
  const imageAlt = `${system.name} — caso de estudio de software de Eduardo Merino`;

  return {
    title,
    description: system.summary,
    alternates: bilingualAlternates(routePath, "es"),
    openGraph: {
      type: "article",
...
export default async function SpanishSystemPage({ params }: SystemPageProps) {
  const { slug } = await params;
  const system = getLocalizedSystemBySlug(slug, "es");
  ...
  const supportingCase = getLocalizedSupportingCase(system.slug, "es");
