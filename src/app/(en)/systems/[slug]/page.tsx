...
export default async function SystemPage({params}: Props) {
 ...
 const polish=getReputationPolish('en');
 ...
 <nav className="case-nav container" aria-label={`${system.name} sections`}><a href="#problem">The problem</a><a href="#what-it-does">What it does</a><a href="#current-state">Current state</a><a href="#engineering">Engineering</a><a href="#evidence">Evidence</a></nav>
 ...
 <section className="state-section section container" id="current-state"><p className="eyebrow accent">Current state</p><div className="split"><h2>{system.state.label}</h2><div className="case-body"><p>{system.state.detail}</p><p className="state-boundary">{system.state.boundary}</p><p className="state-boundary">{polish.system.impactBoundary}</p></div></div></section>
